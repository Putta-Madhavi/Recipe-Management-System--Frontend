...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (mainSearchForm) {
        mainSearchForm.addEventListener('submit', (e) => {
            ...
            if (finalIngredientValue) {
                searchParams.append('ingredient', finalIngredientValue);
            }
            if (mainMealTypeSelect.value) {
                searchParams.append('mealType', mainMealTypeSelect.value);
            }
            if (mainCuisineSelect.value) {
                searchParams.append('cuisine', mainCuisineSelect.value);
            }
            if (mainMaxCookingTimeSelect.value) {
                searchParams.append('maxCookingTime', mainMaxCookingTimeSelect.value);
            }
            if (mainDietaryRequirementSelect.value) {
                searchParams.append('dietaryRequirement', mainDietaryRequirementSelect.value);
            }
    ...
    document.querySelectorAll(".discover-button").forEach(button => {
        button.addEventListener("click", event => {
            ...
            window.location.href = `search-results.html?sort=${encodeURIComponent(sortType)}`;
        });
    });
...
function fetchAllRecipes() {
    ...
    fetch(`${API_BASE_URL}/api/recipes`)
        ...
        .then(data => {
            ...
            if (loadMoreBtn) {
                loadMoreBtn.style.display = data.length > recipesPerPage ? "block" : "none";
            }
        })
        .catch(err => {
...
function filterRecipesByCategory(category) {
    ...
    if (category === "All Types") {
        ...
        currentDisplayedRecipes = allRecipes.filter(recipe => recipe.category === category);
    }
    ...
    if (loadMoreBtn) {
        loadMoreBtn.style.display = currentDisplayedRecipes.length > recipesPerPage ? "block" : "none";
    }
...
function showNextRecipes() {
    ...
    if (loadMoreBtn) {
        loadMoreBtn.style.display = currentIndex >= currentDisplayedRecipes.length ? "none" : "block";
    }
}

function createRecipeCard(recipe) {
    const rating = (recipe.averageRating != null && !isNaN(recipe.averageRating)) ?
        recipe.averageRating.toFixed(1) :
        'N/A';
    const dietary = recipe.dietaryRequirements ?
        `<p><strong>Dietary:</strong> ${recipe.dietaryRequirements}</p>` :
        '';
